	...
	for r in data.get_regions_active():
		...
		for xi in range( region_size ):
			for yi in range( region_size ):
				...
				var h = data.get_height( p );
				var id = data.get_control_base_id( p );
				if( id == 1 ):
					if( randi()%1000 <= 1 ):
						var c = Spawner.spawn( "palm_1", Vector3( p.x, h + randf_range( 0.5, -1.0 ), p.z ), Vector3( deg_to_rad( randf_range( -5.0, 5.0 ) ), randf_range( 0.0, PI*2.0 ), deg_to_rad( randf_range( -5.0, 5.0 ) ) ) );
						#add_child( c );
